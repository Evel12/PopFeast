...
export default function SeriesDetail(){
  ...
  const handleFav=async ()=>{
    ...
    setFav(prev => !prev);
    toggleFavorite({
      id:data.id,title:data.title,type:'series',
      poster_url:data.poster_url,rating:data.rating,
      seasons:data.seasons,episodes:data.episodes
    }).then(async ()=>{
      const all = await getFavorites();
      const isFav = all.some(f=>f.item_id===data.id && f.item_type==='series');
      setFav(isFav);
  ...
  useEffect(()=>{
    setCLoading(true);
    fetch(`/api/series/${id}/comments?__bypass=1&_ts=${Date.now()}`, { headers: { 'Accept': 'application/json', 'x-bypass-cache':'1' }, cache:'no-store' })
      ...
      <div className="detail-hero">
        ...
        <div className="detail-main">
          ...
          <div className="detail-meta">
            {typeof data.seasons==='number' && <div className="meta-chip">{data.seasons} seasons</div>}
            {typeof data.episodes==='number' && <div className="meta-chip">{data.episodes} eps</div>}
